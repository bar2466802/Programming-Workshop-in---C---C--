...
public:
	...
	VLVector() :
				...
				_staticCapacity(N),
				 ...
				 _capacity(N)
	{
		static valueType arr[N]; //Create array on stack
		_arrayStack = arr;
		_array = _arrayStack;
		_arrayHeap = nullptr;
	}
	...
	VLVector(InputIterator first, InputIterator last): _staticCapacity(DEF_CAPACITY)
	{
		...
		_capacity = _size;
		_arrayHeap = nullptr;
		static valueType arr[DEF_CAPACITY]; //Create array on stack
		_arrayStack = arr;
		_array = _arrayStack;
		_onStack = true;
	...
	iterator insert(iterator &it, const valueType value)
	{
		_size++;
		_checkCapacity();
		//Move all the elements from lt location one position to the right
	...
	/**
	 *  Get pointer to the array representing our vector - const version
	 *  @return pointer to the array representing our vector
	 */
