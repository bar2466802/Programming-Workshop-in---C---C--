...
* @param peopleArray - array of people to fill with given info in file
*/
void insertNewPerson(char** input, const int index, FILE** file, PeopleArray** peopleArray)
{
	char endChar = END_LINE;
	(*peopleArray)->counter = index; //update counter of people

	if(peopleArray == NULL || (*peopleArray) == NULL) //check if this is the first time
	{
		//ask for memory for the array
		(*peopleArray)->people = (Person *) malloc(index * sizeof(Person));
	...
	else
	{
		...
		(*peopleArray)->people = (Person *) realloc((*peopleArray)->people, index * sizeof(Person));
		if((*peopleArray)->people == NULL)
		{
			//Something went wrong, exit the program and free dynamic memory
			freeThePeople(&(*peopleArray));
	...
	int result = sscanf
			(
					*input, "%[A-Za-z-] %lu %f%c",
					(*peopleArray)->people[index].name,
					&(*peopleArray)->people[index].id,
					&(*peopleArray)->people[index].age,
					&endChar
			);
	if (result != ARGS_COUNT || endChar != END_LINE) //check the parsing was successful
	{
		//Something went wrong, exit the program and free dynamic memory
		freeThePeople(&(*peopleArray));
		fclose(*file);
...
void maxHeapify(HeapCovidDiagnosis** diagnosis, const Index index)
{
	...
	if(diagnosis == NULL || (*diagnosis) == NULL || (*diagnosis)->count == EMPTY)
	{
		return;
	}
			  ...
			  HeapCovidDiagnosis** diagnosis)
{
	...
	{
		int result = sscanf(*input, "%lu %lu %f %f%c",
							&meeting.sourceId,
							&meeting.targetId,
							&meeting.distance,
							&meeting.time,
...
int main(int argc, char **argv)
{
	...
	HeapCovidDiagnosis* diagnosis = {EMPTY, NULL, NULL};
	//retrieve the people's info from the given file
